...
// 8x8 bitmap font for ASCII characters 32–127 (space through tilde).
// Each character is 8 bytes, one per row, MSB is the leftmost pixel.
// Source: public domain font based on IBM CP437.
...
static void draw_char(char c, uint32_t col, uint32_t row) {
    ...
    for (int y = 0; y < CHAR_HEIGHT; y++) {
        for (int x = 0; x < CHAR_WIDTH; x++) {
            // Test each bit in the glyph row, LSB first (font is stored LSB-left)
