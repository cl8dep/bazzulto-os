...
/// Return the current UART MMIO base address (for use by the TTY layer).
pub fn uart_base_address() -> usize {
    unsafe { UART_BASE }
}
...
/// Print a 64-bit value as `0x<hex>` without requiring the heap or fmt machinery.
///
/// Used by exception handlers where the allocator may not be safe to call.
pub fn put_hex(value: u64) {
    puts("0x");
    // Emit 16 hex digits, most-significant first.
...
pub fn put_dec(value: u64) {
    if value == 0 {
        putc(b'0');
        return;
    }
    // Build digits in reverse order into a small stack buffer.
    let mut digits = [0u8; 20]; // max u64 is 18446744073709551615 (20 digits)
